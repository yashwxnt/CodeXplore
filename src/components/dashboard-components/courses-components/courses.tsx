...
const CourseList = () => {
  ...
  return (
    <div className="mx-auto px-4 py-12">
      <motion.h1
        className="text-4xl font-extrabold mb-12 text-center text-primary leading-tight"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <div className="flex justify-center mb-8">
        <motion.div className="relative w-full sm:w-1/2" initial={{ y: -20 }} animate={{ y: 0 }} transition={{ duration: 0.5 }}>
          <input
            type="text"
            className="w-full border border-input rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
        ...
        <TabsList className="flex justify-center space-x-3 bg-muted p-3 rounded-full shadow-lg overflow-x-auto">
          {categories.map(({ label, icon }) => (
            <TabsTrigger
              key={label}
              value={label}
              className={`flex items-center px-5 py-2 mx-1 rounded-full transition ${
        ...
        {categories.map(({ label }) => (
          <TabsContent key={label} value={label}>
            <div className="flex flex-col sm:flex-row gap-8 mt-8">
              <div className="w-full sm:w-1/5 pr-4 mb-10 sm:mb-0">
